...
const API_ROOT = "http://localhost:8000/public";

// Removal of MOCK_DATA (Strict database-only sourcing)
...
export default function App() {
  ...
  useEffect(() => {
    const batchCode = window.location.pathname.replace(/^\/+/g, '');
    if (!batchCode) {
      setLoading(false);
      return;
    }
    ...
    async function fetchData() {
      try {
        const res = await axios.get(`${API_ROOT}/batch/${batchCode}`);
        setData(res.data);
      } catch (err: any) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#2D0E3D] flex items-center justify-center p-6 text-center">
  ...
  return (
    <div className="max-w-[480px] mx-auto bg-[#2D0E3D] min-h-screen shadow-2xl relative font-sans selection:bg-[#FFA500] selection:text-[#2D0E3D]">

      {/* Hero Video Section */}
      <section className="pt-12 pb-16 px-6 text-center relative overflow-hidden">
        <div className="absolute top-0 inset-x-0 h-40 bg-gradient-to-b from-black/20 to-transparent pointer-events-none z-10" />

        <div className="relative z-20 mb-10">
          <motion.h1
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-[42px] leading-tight font-black text-[#FFA500] font-outfit mb-3 drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)] uppercase tracking-tighter"
          >
        ...
        <div className="relative mx-auto w-full aspect-square mb-12 group cursor-pointer">
          ...
          <div className="absolute inset-x-0 -bottom-6 flex justify-center gap-4 z-30">
            <button className="bg-white/10 backdrop-blur-xl border border-white/20 text-white text-[11px] font-black px-8 py-3 rounded-2xl flex items-center gap-2 hover:bg-[#FFA500] hover:text-[#2D0E3D] transition-all uppercase tracking-widest shadow-xl">
              <User className="w-4 h-4" /> Meet Farmer
            </button>
            <button className="bg-white/10 backdrop-blur-xl border border-white/20 text-white text-[11px] font-black px-8 py-3 rounded-2xl flex items-center gap-2 hover:bg-[#FFA500] hover:text-[#2D0E3D] transition-all uppercase tracking-widest shadow-xl">
      ...
      <section className="px-5 py-16 bg-[#F8F9FB] rounded-t-[4rem] shadow-[0_-30px_60px_rgba(0,0,0,0.4)] relative">
      ...
      <section className="px-5 py-20">
        <div className="bg-[#FFF9EA] rounded-[3.5rem] p-10 border border-[#FFA500]/10 relative overflow-hidden shadow-2xl">
      ...
      <section className="px-5 pb-20 space-y-6">
        {[
          ...
          <div key={sec.id} className={`bg-white rounded-[2.5rem] overflow-hidden shadow-2xl transition-all duration-500 ${openSection === sec.id ? 'ring-2 ring-[#FFA500]' : ''}`}>
            <button
      ...
      <section className="px-5 py-24 text-center bg-black/20 rounded-t-[5rem]">
        <motion.h2
          whileInView={{ opacity: 1, y: 0 }}
          initial={{ opacity: 0, y: 20 }}
          className="text-[42px] leading-tight font-black text-[#FFA500] mb-3 font-outfit uppercase tracking-tighter drop-shadow-lg"
        >
          Forensic <br /> Transparency
        </motion.h2>
        <p className="text-white/30 text-[11px] uppercase font-black tracking-[0.4em] mb-20">Verified Ingredient Efficacy</p>
      ...
      <footer className="p-16 text-center border-t border-white/5 bg-black/40">
        <div className="flex justify-center gap-12 text-white/10 mb-12">
